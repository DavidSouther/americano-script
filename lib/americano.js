...
AmericanoCompiler.prototype = {
    ...
    existentials: function(line){
        ...
        for(var i = 0 ; i < tokens.length ; i++){
            ...
            if (parts = tokens[i].match('^([^\?]+)\?$')) {
                if (tokens[i].indexOf('.') > -1){
                    tokens[i] = '' + parts[1] + ' != null'
                } else {
                    tokens[i] = 'typeof ' + parts[1] +
                        ' !== "undefined" && ' + parts[1] + ' !== null';
                }
                continue;
            }
            ...
            if (tokens[i].indexOf('?') > -1) {
                ...
                varline = varline.substr(0, -2) + ';';
                this.outlines.push(varline);
                tokens[i] = '( '
                for (j = 0; j < parts.length; j++){
                    tokens[i] += '(_ref' + j + ' = ' + parts[j]  + ') != null ? ';
                }
                (_ref = foo.bar) != null ? (_ref1 = _ref.baz) != null ? _ref1.foo : void 0 : void 0;
