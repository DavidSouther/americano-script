...
AmericanoCompiler.prototype = {
    ...
    splices: function(line){
        ...
        spliceRegex = /\[\s*([^\s]*[^\s\.])\s*(\.{2,3})\s*([^\s\]]+)?\s*(?:,\s+([^\s\]]+))?\](?:\s+=\s+([^\s]+|\[[^\]]+\]))?/g;

        line = line.replace(spliceRegex, spliceReplacer);

        return line;

        function spliceReplacer(splice, min, type, max, step, replace, offset) {
            if ( (line.charAt(offset - 1) || ' ').match(/\s/) ) {
                if ( !max ) {
                    throw "No maximum for iterative comprehension"
                }
                step = step || 1;
                var eq = type == '...' ? '=' : '';
                return '(function() { ' +
